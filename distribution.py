...
def common_distribution(dataset, approx_n=None, c1=0, c2=1):
    ...
    checks = np.linspace(np.min(dataset[:,0]), np.max(dataset[:,0]), approx_n + 1)
    checks1 = np.linspace(np.min(dataset[:,1]), np.max(dataset[:,1]), approx_n + 1)
    for i in range(0, approx_n):
...
def show_common_distributions(dataset, eps=0.2):
    ...
    N = int(math.ceil(np.sqrt(len(pairs))))
    ...
    for n, (i, j) in enumerate(pairs):
        ...
        # density = density[1:-1, 1:-1]
        extent = [
            np.min(dataset[:, :, :, j]), np.max(dataset[:, :, :, j]),
            np.min(dataset[:, :, :, i]), np.max(dataset[:, :, :, i]),
        ]
        ax = fig.add_subplot(N, N, n + 1)
        ax.imshow(density[::-1, :],
